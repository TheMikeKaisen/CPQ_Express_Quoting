...
import importProductsData from
    '@salesforce/apex/ProductController.importProductsData';
...
export default class ProvusProductsList extends LightningElement {
    ...
    processCsv(csvStr) {
        ...
        importProductsData({ jsonData: JSON.stringify(data) })
            .then(() => {
                if (this.wiredProductsResult) refreshApex(this.wiredProductsResult);
            })
            .catch(error => {
                const msg = error.body ? error.body.message : 'Error importing products';
                // eslint-disable-next-line no-alert
